  ...
  <view class="container">
    ...
    <view v-else>
      ...
      <view class="thing">
        <view>
          共拉了
          <text class="thing__count">{{ dataInfo.thing }}</text>
          坨
        </view>
        <image
      ...
      <!-- 刷题 -->
      <view class="problem">
        <view class="problem-time problem-item">
          做题时长为
          <text class="problem__count problem-time__count">{{ formateDateHMS(dataInfo.useTime || 0) }}</text>
        </view>
        <view class="problem-count problem-item">
          刷题总数量为
          <text class="problem__count problem-count__count">{{ dataInfo.totalNumber }}</text>
          道
        </view>
        <view class="probelm-true problem-item">
          正确率为
      ...
      <view class="say">
        <view class="say-get">
          共收到别人的悄悄话
          <text class="say-get__count">{{ dataInfo.wantSayGet }}</text>
          条
        </view>
        <view class="say-send">
          共发送悄悄话
          <text class="say-send__count">{{ dataInfo.wantSaySend }}</text>
          条
        </view>
      </view>
    </view>
...
/**
 * 总结报告页
 * 展示指定时间段内的拉屎次数、做题时长、做题数量、正确率和悄悄话统计
 */
