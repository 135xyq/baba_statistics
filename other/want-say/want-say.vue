...
export default {
  ...
  methods: {
    ...
    onAddPage() {
      uni.navigateTo({
        url: "/other/want-say-add/want-say-add?type=add",
      });
    },
    /**
     * 长按内容，出现编辑和删除的操作弹窗
    ...
    onHandleSelectAction(e) {
      ...
      if (name === "修改") {
        uni.navigateTo({
          url: `/other/want-say-add/want-say-add?type=edit&data=${JSON.stringify(this.currentInfo)}`,
        });
      } else if (name === "删除") {
        uni.showModal({
          title: "提示",
