<template>
	<view class="stopwatch-container">
		<!-- 底部tab -->
		<view>
			<u-tabbar activeColor="#d81e06" @change="clickTab" :value="tabValue" :fixed="true" :placeholder="true"
				:safeAreaInsetBottom="true">
				<u-tabbar-item text="秒表" icon="clock">
				</u-tabbar-item>
				<u-tabbar-item text="计时器" icon="calendar">收到给我钱</u-tabbar-item>
			</u-tabbar>
		</view>
		<!-- 秒表 -->
		...
		<view v-if="tabValue === 1">
			<u-count-down ref="countDown" :time="initTime" format="HH:mm:ss" :autoStart="false" millisecond
				@change="onChange" @finish="onTimeFinish">
				<view class="time">
					<view class="time__custom">
			...
			<view class="buttons time-buttons">
				...
				<button @click="onStartTime" v-if="!isRunningTime">开始</button>
				<button class="stop" @click="onPauseTime" v-if="isRunningTime">暂停</button>
			</view>
		</view>
	</view>
</template>
